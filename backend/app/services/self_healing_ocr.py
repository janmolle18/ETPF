...
def _is_valid_healing_candidate(
    orig_text: str, candidate_text: str, orig_conf: float, candidate_conf: float
) -> bool:
    """
    Validates sub-crop OCR results to ensure self-healing never corrupts
    valid words or introduces hallucinations.
    """
    import re
    ...
    # Candidates should not replace numbers/words with completely unrelated
    # text (e.g. s0.00 -> 8000 or PRIMECORP -> PRIHECORP)
    if sim < 0.40 and orig_conf >= 0.25:
        return False

    # Rule 3: If original text had medium confidence (>= 0.50),
    # candidate must retain structural similarity (>= 0.75)
...
def optimize_low_confidence_blocks(
    ...
    for block in target_blocks:
        ...
        # Tactic 2: 2x Interpolated zoom + Grayscale contrast enhancement
        # (combats small characters/noise without hard binarization)
        w, h = crop_img.size
        ...
        if best_tactic_conf > orig_conf and best_tactic_text != orig_text:
            ...
            # Track block-specific optimization boost
            block["confidence_gain"] = round(gain * 100, 1)
            # Keep record of original text for visual before/after comparison
    ...
    optimization_stats = {
        ...
        "avg_confidence_gain": (
            round((total_gain / healed_count) * 100, 1) if healed_count > 0 else 0.0
        ),
